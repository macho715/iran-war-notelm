...
Usage:
  1. Export urgentdash data to JSON (see docs/URGENTDASH_BACKUP.md).
  2. python scripts/backup_urgentdash.py [path/to/snapshot.json]
     Default input: urgentdash/urgentdash_snapshot.json
"""
...
def main() -> int:
    ...
    parser.add_argument(
        "snapshot",
        nargs="?",
        default=str(ROOT / "urgentdash" / "urgentdash_snapshot.json"),
        help="Path to urgentdash JSON snapshot (default: urgentdash/urgentdash_snapshot.json)",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    if not path.exists():
        ...
        print("Export urgentdash data to JSON first (see docs/URGENTDASH_BACKUP.md).", file=sys.stderr)
        return 1
